...
export default function App() {
  ...
  const [locks, setLocks] = useState({}); // Default state: all items UNLOCKED (empty object)
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(lockDoc, (snapshot) => {
      if (snapshot.exists()) {
        ...
        // If doc doesn't exist, we keep locks as an empty object (all unlocked)
        setLocks({});
      }
  ...
  const startTest = (setIdx) => {
    ...
    // Content is only inaccessible if explicitly locked in Firestore AND user is not Admin
    if (isLocked(lockKey) && !adminAuth) {
      return; // Do nothing if locked
    }
    const qList = getQuestions(selectedSubject.id, selectedChapterIdx, setIdx);
    if (qList.length === 0) {
      // We only alert if there is actually no data yet
      return;
    }
  ...
  const handleAdminAuth = () => {
    if (accessCodeInput === '9563') {
      ...
      // Custom modal logic instead of alerts
    }
  };
